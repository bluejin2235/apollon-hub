...
#   03:25 LUNA Work Text Embed    — 기본 비활성 (trigram 우선; 증거 나온 뒤 켤 것)
$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent $PSScriptRoot
$ExtractBat = Join-Path $Root "run-nas-text-incremental.bat"
$EmbedBat = Join-Path $Root "run-nas-text-embed.bat"

function Register-DailyIT([string]$Name, [string]$BatPath, [string]$Time, [switch]$Disabled) {
  ...
  if ($Disabled) {
    ...
    Write-Host "Registered $Name daily at $Time (IT, DISABLED)"
  } else {
...
Register-DailyIT "LUNA Work Text Embed" $EmbedBat "03:25" -Disabled
