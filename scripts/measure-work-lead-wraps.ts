/**
 * 워크 기본 설명 — 공개 vs 편집기 줄바꿈 글자 단위 비교
 * npx tsx scripts/measure-work-lead-wraps.ts
 */
...
function measureWraps() {
  return (el: Element) => {
    ...
    return {
      ...
      elWidth: Math.round(er.width * 10) / 10,
      padding: cs.padding,
      ...
      htmlHead: p.innerHTML.slice(0, 240)
    };
  };
}
...
async function main() {
  ...
  await page.locator(".content__heading").first().screenshot({
    path: resolve(OUT, `wrap-public-${tag}.png`)
  });
  ...
  const edAsIs = await page.locator(".rte-ed--work-lead").first().evaluate(measureWraps());
  await page.locator(".rte-ed--work-lead").first().screenshot({
    path: resolve(OUT, `wrap-editor-${tag}-asis.png`)
  });

  /* 같은 본문(굵기 없음)으로 맞춘 뒤 — 서식 차이로 인한 줄바꿈 오차 제거 */
  const plain = pub.lines.join("");
  const edPlain = await page.locator(".rte-ed--work-lead").first().evaluate((el, text) => {
    ...
    return {
      ...
      lines,
      ends: lines.map((l) => l.slice(-12)),
      htmlHead: p.innerHTML.slice(0, 120)
    };
  }, plain);
  await page.locator(".rte-ed--work-lead").first().screenshot({
    path: resolve(OUT, `wrap-editor-${tag}-plain.png`)
  });
  ...
  const report = {
    ...
    editorAsIs: edAsIs,
    editorPlain: edPlain,
    asIsEndsMatch: JSON.stringify(pub.ends) === JSON.stringify(edAsIs.ends),
    plainEndsMatch: JSON.stringify(pub.ends) === JSON.stringify(edPlain.ends),
    asIsLineCountMatch: pub.lineCount === edAsIs.lineCount,
    plainLineCountMatch: pub.lineCount === edPlain.lineCount
  };
  writeFileSync(resolve(OUT, `wrap-compare-${tag}.json`), JSON.stringify(report, null, 2));
