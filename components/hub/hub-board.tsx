...
export function HubBoard() {
  ...
  const fetchPostsPage = useCallback(async (page: number) => {
    ...
    if (authorIds.length > 0) {
      ...
      if (pe) console.error("[hub_posts] profiles", pe);
    ...
    if (postIds.length > 0) {
      const { data: comments, error: ce } = await supabase.from("hub_comments").select("post_id").in("post_id", postIds);
      if (ce) {
        console.error("[hub_comments]", ce);
      } else {
  ...
  return (
    <>
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5" aria-labelledby="hub-board-heading">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
          <div className="min-w-0">
            <h2 id="hub-board-heading" className="text-base font-bold tracking-tight text-slate-900 sm:text-lg">
              Hub 게시판
            </h2>
          ...
          <table className="w-full min-w-0 text-left text-sm">
            <thead className="border-b border-slate-100 bg-slate-50/90 text-xs font-semibold text-slate-600">
              <tr>
                ...
                <th className="hidden w-[7.5rem] shrink-0 px-2 py-2.5 md:table-cell">작성일</th>
                    ...
                    <tr key={row.id} className="hover:bg-slate-50/80">
                      <td className="max-w-0 px-3 py-3 sm:px-4">
                        ...
                        <p className="mt-0.5 text-[11px] text-slate-500 sm:hidden">
                          {row.authorName} · {formatPostDate(row.created_at)}
                        </p>
                      </td>
                      <td className="hidden px-2 py-3 text-slate-700 sm:table-cell">{row.authorName}</td>
                      <td className="hidden whitespace-nowrap px-2 py-3 text-xs text-slate-600 md:table-cell">
        ...
        {totalPages > 1 ? (
          <nav className="mt-3 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-sm text-slate-600" aria-label="Hub 게시판 페이지">
            <button
              type="button"
              aria-label="이전 페이지"
