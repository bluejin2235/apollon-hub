...
export function AshulengPostWriteModal({ open, authorProfileId, editingPost = null, onClose, onSaved }: Props) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal
      aria-labelledby="ashuleng-write-title"
      onClick={onClose}
    >
      <div
      ...
      >
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
          <h2 id="ashuleng-write-title" className="text-lg font-bold text-slate-900">
            {isEdit ? "글 수정" : "글쓰기"}
          </h2>
          <button
            type="button"
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-5">
          <div>
            <label htmlFor="ashuleng-post-title" className="mb-1 block text-xs font-semibold text-slate-700">
              제목
            </label>
            <input
              id="ashuleng-post-title"
              value={title}
          ...
          <div>
            <label htmlFor="ashuleng-post-content" className="mb-1 block text-xs font-semibold text-slate-700">
              내용
            </label>
            <textarea
              id="ashuleng-post-content"
              value={content}
