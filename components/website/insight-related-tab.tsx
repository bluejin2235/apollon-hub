...
const HELP = {
  title: "관련 콘텐츠",
  use: "인사이트 상세 맨 아래 「Related Articles」 자리. 카드 제목은 연결한 콘텐츠의 제목을 그대로 가져옵니다",
  rule: "4개. 워크 · 인사이트 · 정적 페이지를 섞어 넣을 수 있습니다",
  note: "같은 분류만 넣지 마세요. 워크를 하나 이상 섞으면 읽을거리가 생겨 체류 시간이 늘어납니다",
...
export function InsightRelatedTab({ insight, siteUrl, onReload }: Props) {
  const savedSlots = useMemo(() => slotsFromRelated(insight.content_related), [insight.content_related]);
  ...
  useEffect(() => {
    ...
    async function hydrate() {
      ...
      const missingInsights = (insight.content_related ?? [])
        .filter((item) => item.target_type === "insight" && item.target_insight_id)
        .map((item) => item.target_insight_id as string)
        .filter((id) => !map.has(`insight:${id}`));

      await Promise.all(
        missingInsights.map(async (id) => {
          const res = await getInsight(id);
          if (!res.ok) return;
          const detail = parseInsightDetail(res.data);
          if (!detail) return;
          map.set(`insight:${detail.id}`, {
            type: "insight",
            id: detail.id,
            title: detail.title,
            slug: detail.slug,
  ...
  const excludeKeys = useMemo(() => new Set<string>([`insight:${insight.id}`]), [insight.id]);
  ...
  async function recommend() {
    setLuna("loading");
    setError(null);
    const res = await recommendRelated("", { insightId: insight.id });
    if (!res.ok || !res.data) {
      setLuna("error");
      return;
    }
  ...
  async function save() {
    setSaveState("saving");
    setError(null);
    const saved = [...(insight.content_related ?? [])];
    const filled = slots
    ...
    for (const row of saved) {
      ...
      if (!still) {
        const res = await deleteInsightRelated(insight.id, row.id);
        if (!res.ok) {
          setError(res.error);
          setSaveState("dirty");
          return;
    ...
    for (const slot of filled) {
      ...
      const res = await addInsightRelated(insight.id, relatedBody(slot, slot.sort));
      if (!res.ok) {
        setError(res.error);
        setSaveState("dirty");
        return;
    ...
    if (created.length > 0) {
      const res = await reorderInsightRelated(
        insight.id,
  ...
  return (
    <div className="wa">
      ...
      <p className="grpd">상세 맨 아래에 이 순서대로 4개가 나옵니다. 워크와 인사이트를 섞을 수 있습니다.</p>
      ...
      <div className="rel-luna-row">
        ...
        <div className="rel-luna-txt">
          {luna === "loading" ? (
            ...
            "이 인사이트와 어울리는 콘텐츠를 루나가 골라 줄 수 있습니다."
          )}
        </div>
