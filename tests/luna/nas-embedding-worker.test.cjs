...
async function run({apply=true,args=[],current=[row],storeResult={data:[row.id],error:null},providerError=null,readError=false}={}){
 ...
 const deps={
  ...
  '@/lib/luna/nas-text-runs':{startNasTextRun:async()=>{events.push({start:true});return 'run'},
